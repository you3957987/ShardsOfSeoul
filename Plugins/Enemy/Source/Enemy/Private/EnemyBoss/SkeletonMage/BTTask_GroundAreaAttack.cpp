...
{
	NodeName = "SkeletonMageGroundAreaAttack";
...
EBTNodeResult::Type UBTTask_GroundAreaAttack::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	Boss->PlayGroundAreaAttackMontage();
	bIsAttacking = true;
...
void UBTTask_GroundAreaAttack::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	...
	if (Boss && !Boss->GetMesh()->GetAnimInstance()->Montage_IsPlaying(Boss->GroundAreaAttackMontage))
	{
